...
figure(1);
 plot( a, betapdf(betainv(a,alfa,beta),alfa,beta).^(-1),'k-.') % observed f^Y
hold on
plot(a,linear_est1,'b')
title(['Linear wavelet quantile density Beta(' num2str(alfa),',' num2str(beta),'),n=',num2str(n) ])



% How does the linear estimators of quantile densities look like after hard thresholding?
figure(2);
 plot( a, 1./betapdf(betainv(a,alfa,beta),alfa,beta),'k-.') % observed f^Y
hold on
plot(a, smooth_th1,'r')
title(['Linear wavelet estimator after thresholding-quantile density Beta(' num2str(alfa),',' num2str(beta),'),n=',num2str(n) ])
...
title(['Smoothed linear wavelet estimator-quantile density Beta(' num2str(alfa),',' num2str(beta),'),n=',num2str(n) ])
% How does it look like three different estimators of quntile density?
    %--------------------------------------------------------------------------
    figure(4)
plot(a, 1./betapdf(betainv(a,alfa,beta),alfa,beta),'k', a,linear_est1,'b:',a, smooth_th1,'r-.', (1:n)/(n+1), smooth_ll, 'g--',a2, Jons(y,h),'yo',a2,SDJ(y,h),'mx',...
'MarkerSize',2);
%                 'LineWidth',1,...
% %                 'MarkerEdgeColor','m',...
% %                 'MarkerFaceColor','w',...
...
%                 'MarkerFaceColor','w',...
%                 'MarkerSize',5);
title(['Estimation of quantile density Beta(' num2str(alfa),',' num2str(beta),'),n=',num2str(n) ])
